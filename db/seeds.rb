...
20.times do |index|	#cree 20 gossip en reference avec user
	Gossip.create(title: "Gossip#{index}",content: Faker::GreekPhilosophers.quote, user: User.all.sample)
end
...
20.times do	#creer 20 messages en reference avec l'utilsateur sender et creer un nombre aleatoire de receveur user
	message = PrivateMessage.create(sender: User.all.sample, content: Faker::Movies::BackToTheFuture.quote)
	rand(1..6).times do
...
20.times do		#cree 20 comments
	Comment.create(content: Faker::Books::Dune.saying, user: User.all.sample, gossip: Gossip.all.sample)
end
